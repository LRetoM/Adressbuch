...
function importContactsFromCSV() {
    ...
    fs.createReadStream(csvFilePath)
        ...
        .on('data', (row) => {
            ...
            db.run(`INSERT INTO contacts (first_name, last_name, city, street, house_number, phone_number) VALUES (?, ?, ?, ?, ?, ?)`, 
                    [vorname, nachname, ort, strasse, hausnummer, telefonnummer], 
                    (err) => {
                        if (err) {
...
app.post('/contacts', (req, res) => {
    ...
    const checkSql = `SELECT * FROM contacts WHERE first_name = ? AND last_name = ? AND city = ? AND street = ? AND house_number = ? AND phone_number = ?`;
    db.get(checkSql, [first_name, last_name, city, street, house_number, phone_number], (err, row) => {
        if (err) {
            res.status(500).json({ error: 'Datenbankfehler' });
        } else if (row) {
            // Kontakt existiert bereits
            res.status(409).json({ error: 'Ein Kontakt mit diesen Informationen existiert bereits.' });
        } else {
            // Füge den neuen Kontakt hinzu
            const insertSql = `INSERT INTO contacts (first_name, last_name, city, street, house_number, phone_number) VALUES (?, ?, ?, ?, ?, ?)`;
            db.run(insertSql, [first_name, last_name, city, street, house_number, phone_number], function(err) {
                if (err) {
                    res.status(500).json({ error: 'Fehler beim Einfügen' });
                } else {
                    res.status(201).json({ message: 'Kontakt erfolgreich hinzugefügt', id: this.lastID });
                }
            });
        }
    });
...
app.put('/contacts/:id', (req, res) => {
    ...
    db.run(sql, values, function(err) {
        ...
        res.json({ message: 'Kontakt aktualisiert', contact: { id, ...req.body } });
    });
});
